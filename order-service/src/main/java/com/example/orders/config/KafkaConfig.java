...
import com.example.orders.util.Json;
...
public class KafkaConfig {
    ...
    public ConsumerFactory<String, EventEnvelope> consumerFactory() {
        ...
        JsonDeserializer<EventEnvelope> valueDeserializer = new JsonDeserializer<>(EventEnvelope.class, Json.mapper());
    ...
    public ConcurrentKafkaListenerContainerFactory<String, EventEnvelope> kafkaListenerContainerFactory(
        ...
        // Retry policy (shows "failure handling vs complexity")
        ExponentialBackOff backOff = new ExponentialBackOff(500L, 2.0);
        backOff.setMaxInterval(5000L);

        DefaultErrorHandler errorHandler = new DefaultErrorHandler(backOff);
        // After retries are exhausted, message remains uncommitted and will be re-consumed.
        // In the next iteration we can wire a DLQ topic.
